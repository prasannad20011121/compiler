...
public class TDecimalFormat extends TNumberFormat {
    ...
    public TDecimalFormat(String pattern, TDecimalFormatSymbols value) {
        symbols = copySymbols(value);
        applyPattern(pattern);
    }

    /**
     * Manual field-by-field copies, used instead of {@code (TDecimalFormatSymbols) x.clone()} everywhere
     * in this class. Casting the result of {@code Object.clone()} back to its own concrete type trips a
     * WASM-GC backend bug in this TeaVM version (confirmed by isolated repro: the clone() call itself
     * returns non-null, but the subsequent cast produces a reference that null-derefs on the very next
     * field access) — this is what made every printf/String.format call crash with "dereferencing a
     * null pointer" a few frames further in, inside DecimalFormat.forDigit(). Working around it here by
     * never calling clone() (or casting its result) sidesteps the backend bug entirely rather than
     * fixing it, but it's a safe, narrow, classlib-only change — no core/backend code is touched.
     *
     * Two overloads because this classlib module compiles as ordinary Java against a real JDK: real
     * java.text.DecimalFormatSymbols / java.util.Locale (used by this class's public API, so external
     * callers stay source-compatible with java.text.DecimalFormat) are genuinely distinct types from
     * TDecimalFormatSymbols / TLocale (used internally) here, even though TeaVM's target-side class
     * substitution treats same-named real/T-prefixed classes as one type once compiled to Wasm.
     */
    private static TDecimalFormatSymbols copySymbols(TDecimalFormatSymbols src) {
        TDecimalFormatSymbols copy = new TDecimalFormatSymbols(src.getLocale());
        copyFields(src.getZeroDigit(), src.getGroupingSeparator(), src.getDecimalSeparator(), src.getPerMill(),
                src.getPercent(), src.getDigit(), src.getPatternSeparator(), src.getNaN(), src.getInfinity(),
                src.getMinusSign(), src.getMonetaryDecimalSeparator(), src.getExponentSeparator(), copy);
        return copy;
    }

    private static TDecimalFormatSymbols copySymbols(DecimalFormatSymbols src) {
        java.util.Locale locale = src.getLocale();
        TLocale tLocale = locale == null ? TLocale.getDefault()
                : new TLocale(locale.getLanguage(), locale.getCountry(), locale.getVariant());
        TDecimalFormatSymbols copy = new TDecimalFormatSymbols(tLocale);
        copyFields(src.getZeroDigit(), src.getGroupingSeparator(), src.getDecimalSeparator(), src.getPerMill(),
                src.getPercent(), src.getDigit(), src.getPatternSeparator(), src.getNaN(), src.getInfinity(),
                src.getMinusSign(), src.getMonetaryDecimalSeparator(), src.getExponentSeparator(), copy);
        return copy;
    ...
    private static void copyFields(char zeroDigit, char groupingSeparator, char decimalSeparator, char perMill,
            char percent, char digit, char patternSeparator, String naN, String infinity, char minusSign,
            char monetaryDecimalSeparator, String exponentSeparator, TDecimalFormatSymbols copy) {
        copy.setZeroDigit(zeroDigit);
        copy.setGroupingSeparator(groupingSeparator);
        copy.setDecimalSeparator(decimalSeparator);
        copy.setPerMill(perMill);
        copy.setPercent(percent);
        copy.setDigit(digit);
        copy.setPatternSeparator(patternSeparator);
        copy.setNaN(naN);
        copy.setInfinity(infinity);
        copy.setMinusSign(minusSign);
        copy.setMonetaryDecimalSeparator(monetaryDecimalSeparator);
        copy.setExponentSeparator(exponentSeparator);
    }
    ...
    public DecimalFormatSymbols getDecimalFormatSymbols() {
        // Type-system-only cast (real DecimalFormatSymbols and TDecimalFormatSymbols are unrelated types
        // to a real javac compiling this module, but the same class once substituted for the Wasm
        // target) — copySymbols() itself never touches the buggy clone()+cast path this works around.
        return (DecimalFormatSymbols) (Object) copySymbols(symbols);
    }

    public void setDecimalFormatSymbols(DecimalFormatSymbols symbols) {
        this.symbols = copySymbols(symbols);
    }
