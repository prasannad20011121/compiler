...
public abstract class BaseWasmGenerationVisitor implements StatementVisitor, ExprVisitor {
        ...
        switch (expr.getOperation()) {
            ...
            case DIVIDE:
                if (expr.getType() == OperationType.INT || expr.getType() == OperationType.LONG) {
                    result = generateIntDivisionWithZeroCheck(WasmIntBinaryOperation.DIV_SIGNED, expr);
                } else {
            ...
            case MODULO: {
                switch (expr.getType()) {
                    case INT:
                    case LONG:
                        result = generateIntDivisionWithZeroCheck(WasmIntBinaryOperation.REM_SIGNED, expr);
                        break;
                    default:
    ...
    // Integer division and remainder trap at the raw Wasm instruction level (i32.div_s/i64.div_s,
    // i32.rem_s/i64.rem_s) on division by zero - an uncatchable low-level trap, unlike the JVM's
    // catchable ArithmeticException("/ by zero"). Unlike ADD/SUB/MUL/etc, DIVIDE and (integer)
    // MODULO need an explicit managed check-and-throw wrapper.
    //
    // This MUST use a real if/then/else (WasmConditional, as visit(ConditionalExpr) above does for
    // the ?: operator) rather than the "branch with result" pattern nullCheck()/visit(BoundCheckExpr)
    // use below: those are safe with a branch-with-result because their "result" value is just an
    // already-computed operand (a plain local read), safe to evaluate unconditionally regardless of
    // which arm is taken. Here the "result" IS the division itself - the very operation whose safety
    // depends on the branch condition - so it must live in the WasmConditional's thenBlock, only
    // evaluated once the divisor is confirmed nonzero, not computed eagerly before any check runs.
    private WasmExpression generateIntDivisionWithZeroCheck(WasmIntBinaryOperation intOp, BinaryExpr expr) {
        var intType = expr.getType() == OperationType.LONG ? WasmIntType.INT64 : WasmIntType.INT32;
        var wasmType = expr.getType() == OperationType.LONG ? WasmType.INT64 : WasmType.INT32;

        var setupBlock = new WasmBlock(false);
        setupBlock.setLocation(expr.getLocation());

        accept(expr.getFirstOperand());
        var dividend = exprCache.create(result, wasmType, expr.getLocation(), setupBlock.getBody());

        accept(expr.getSecondOperand());
        var divisor = exprCache.create(result, wasmType, expr.getLocation(), setupBlock.getBody());

        var zero = intType == WasmIntType.INT64 ? new WasmInt64Constant(0) : new WasmInt32Constant(0);
        var nonZeroCond = new WasmIntBinary(intType, WasmIntBinaryOperation.NE, divisor.expr(), zero);

        var conditional = new WasmConditional(nonZeroCond);
        conditional.setType(wasmType.asBlock());
        conditional.setLocation(expr.getLocation());

        var division = new WasmIntBinary(intType, intOp, dividend.expr(), divisor.expr());
        division.setLocation(expr.getLocation());
        conditional.getThenBlock().getBody().add(division);
        conditional.getThenBlock().setType(wasmType.asBlock());

        var callSiteId = generateCallSiteId(expr.getLocation());
        callSiteId.generateRegister(conditional.getElseBlock().getBody(), expr.getLocation());
        generateThrowArithmeticException(expr.getLocation(), conditional.getElseBlock().getBody());
        callSiteId.generateThrow(conditional.getElseBlock().getBody(), expr.getLocation());
        conditional.getElseBlock().setType(wasmType.asBlock());

        dividend.release();
        divisor.release();

        setupBlock.getBody().add(conditional);
        setupBlock.setType(wasmType.asBlock());
        return setupBlock;
    }

    protected abstract void generateThrowArithmeticException(TextLocation location, List<WasmExpression> target);
