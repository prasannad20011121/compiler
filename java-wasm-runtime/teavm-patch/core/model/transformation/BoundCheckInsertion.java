...
public class BoundCheckInsertion {
    ...
    static class InsertionVisitor extends AbstractInstructionVisitor
        ...
        private void tryInsertBoundCheck(Variable indexVar, Variable arrayVar, Instruction instruction) {
            ...
            if (isConstant[index] && isConstantSizedArray[array]) {
                // Only elide the check if the constant index is actually provably within the
                // constant array size — previously this branch dropped the upper-bound check
                // whenever *both* were merely constants, without comparing their values, so a
                // literal out-of-bounds access (e.g. `new int[3]` then `arr[5]`) silently lost
                // its bounds check. The access then fell through to the raw array.get/array.set
                // Wasm instruction, which still traps on out-of-bounds — but as an uncatchable
                // Wasm trap instead of a catchable ArrayIndexOutOfBoundsException, since it
                // bypassed TeaVM's own managed-exception-throwing bound check entirely.
                if (constantValue[index] >= 0 && constantValue[index] < constantValue[array]) {
                    upper = false;
                }
            }
            if (upper) {
