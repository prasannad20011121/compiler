 ...
 * Uses the vendored teavm-javac toolchain (Apache-2.0): real javac 21 +
 * TeaVM compiled to WASM-GC, running entirely in the browser.
 *
...
function ensureCompiler() {
  compilerReady ??= (async () => {
    post({ type: 'system', text: 'loading Java 21 toolchain (first run only, ~7 MB)…' });
